...
router.post('/register', async(req, res) => {
    try {
        ...
        if (existingEmail) {
            return res.status(400).json({ message: `Account with ${existingemail} already exists` });
        }

        const existingUsername = await User.findOne({ username });
        if (existingUsername) {
            return res.status(400).json({ message: `Account with ${existingUsername} already exists` });
        }
